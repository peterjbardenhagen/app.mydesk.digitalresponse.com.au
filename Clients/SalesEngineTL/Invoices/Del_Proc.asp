...
sql = "Select InvoiceStatusId From Invoices Where InvoiceId = " & lngId
...
If Request.Cookies("UserSettings")("UserTypeId") = 5 Or Request.Cookies("UserSettings")("UserTypeId") = 6 Then
	boolCanDelete = True
Else
	If rsCheck("InvoiceStatusId") = 1 Then
		boolCanDelete = True
	Else
		boolCanDelete = False
	End If
...
If boolCanDelete Then
	sql = "Delete From Comments Where ItemId = " & lngId & " And TableId = 10"
	dbConn.Execute(sql)

	sql = "Delete From InvoiceContents Where InvoiceId = " & lngId
	dbConn.Execute(sql)

	sql = "Delete From InvoiceAudit Where InvoiceId = " & lngId
	dbConn.Execute(sql)

	sql = "Delete From Invoices Where InvoiceId = " & lngId
	dbConn.Execute(sql)
	...
	End If
Else
	strMsg = "Cannot delete this Invoice, as it has been issued."
End If
