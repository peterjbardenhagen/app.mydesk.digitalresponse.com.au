...
public class ProductService
{
    ...
    public async Task<List<Product>> GetAllAsync()
    {
        var dt = await _db.QueryAsync(@"
            SELECT ProductId, Product AS ProductName, Description,
                   UnitCost, UnitPrice
    ...
    {
        var dt = await _db.QueryAsync(
            "SELECT TOP 1 ProductId, Product AS ProductName, Description, UnitCost, UnitPrice FROM Products WHERE ProductId = @id",
            new() { ["id"] = id });
    ...
    public async Task<int> SaveAsync(Product p)
    {
        if (p.ProductId == 0)
        {
            return await _db.InsertAsync(
                @"INSERT INTO Products (Product, Description, UnitCost, UnitPrice)
        ...
        await _db.ExecuteAsync(
            @"UPDATE Products SET Product=@Name, Description=@Desc, UnitCost=@Cost, UnitPrice=@Price
              WHERE ProductId = @Id",
            new()
            {
                ["Id"] = p.ProductId,
