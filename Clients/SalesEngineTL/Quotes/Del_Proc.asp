...
sql = "Select QuoteStatusId From Quotes Where Qid = " & lngId
...
If Request.Cookies("UserSettings")("UserTypeId") = 5 Or Request.Cookies("UserSettings")("UserTypeId") = 6 Then
	boolCanDelete = True
Else
	If rsCheck("QuoteStatusId") = 1 Then
		boolCanDelete = True
	Else
		boolCanDelete = False
	End If
...
If boolCanDelete Then
	sql = "Delete From Comments Where ItemId = " & lngId & " And TableId = 6"
	dbConn.Execute(sql)

	sql = "Delete From QuoteThirdPartyContents Where QuoteId = " & lngId
	dbConn.Execute(sql)

	sql = "Delete From QuoteContents Where Qid = " & lngId
	dbConn.Execute(sql)

	sql = "Delete From QuoteAudit Where Qid = " & lngId
	dbConn.Execute(sql)

	sql = "Delete From Quotes Where Qid = " & lngId
	dbConn.Execute(sql)
	...
	End If
Else
	strMsg = "Cannot delete this Quote, as it has been issued."
End If
