...
function activewidgets_grid(name, oRecordset)
	...
	Do while (Not oRecordset.eof)
		...
		For i=0 to (column_count-1)
			If i = (column_count-1) Then
				rows = rows & """" & activewidgets_html(oRecordset("TMail #"), oRecordset(i), oRecordset(i).name) & """ "
			Else
				rows = rows & """" & activewidgets_html(oRecordset("TMail #"), oRecordset(i), oRecordset(i).name) & """, "
			End If
		Next
...
function activewidgets_html(Id, s, FieldName)
	...
	If Not IsNull(s) Then
		...
		ElseIf s = "ACTION" Then
			s = "<a href='#' onclick='ViewTMail(""" & Request.Cookies("ClientSettings")("WorkingDir") & """, " & Id & ");'>View</a> | <a href='Del_Proc.asp?TMailId=" & Id & "' target='_parent'>Delete</a>"
		End If
		s = Replace(s, "\", "\\")
		s = Replace(s, """", "\""")
		s = Replace(s, vbCr, "\r")
...
' Execute a SQL query
sql = "SELECT TMailId As [TMail #], Users.Name As [From], Users_1.Name As [To], 'ACTION' As [Action], TMail.Subject, TMail.Message, Read, TMail.Date FROM (TMail INNER JOIN Users ON TMail.FromCode = Users.Code) INNER JOIN Users AS Users_1 ON TMail.ToCode = Users_1.Code WHERE TMail.Date >= #" & dteDateFrom & "# AND TMail.Date <= #" & dteDateTo & "#"
If strCode = "All" Then
	sql = sql & " AND TMail.ToCode In (" & GetAccessCodesList(Request.Cookies("UserSettings")("Code"), Request.Cookies("UserSettings")("UserTypeID")) & ")"
Else
	sql = sql & " AND TMail.ToCode = '" & strCode & "'"
End If
