...
public class InvoiceService
{
    ...
    private const string SelectCols = @"
        i.InvoiceId,
        ISNULL(i.InvoiceNum,  '')  AS InvoiceNum,
        i.InvoiceDate,
        ...
        i.ContactId,
        ISNULL(co.Company, ISNULL(i.CCompany,'')) AS CCompany,
    ...
    public async Task<List<InvoiceLineItem>> GetLineItemsAsync(int invoiceId)
    {
        var dt = await _db.QueryAsync(@"
            SELECT InvoiceContentId, InvoiceId,
            ...
            WHERE InvoiceId = @Id
            ORDER BY InvoiceContentId",
            new() { ["Id"] = invoiceId });
        return dt.Map(r => new InvoiceLineItem
        {
            InvoiceContentId = Convert.ToInt32(r["InvoiceContentId"]),
    ...
    public async Task<int> CreateInvoiceAsync(Invoice inv, List<InvoiceLineItem> lines, string userCode)
    {
        var id = await _db.InsertAsync(@"
            INSERT INTO Invoices (InvoiceNum, InvoiceDate, Code, InvoiceStatusId, DivisionId,
                Qid, CompanyId, ContactId, CCompany, InvCompany, DelCompany,
                InvAddress, DelAddress, CustomerPO, Attention, Account,
                Terms, CustomerNotes, InternalNotes, NettPriceTotal, GSTTotal)
            VALUES (@InvoiceNum, @InvoiceDate, @Code, 1, @DivisionId,
                @Qid, @CompanyId, @ContactId, @CCompany, @InvCompany, @DelCompany,
                @InvAddress, @DelAddress, @CustomerPO, @Attention, @Account,
                @Terms, @CustomerNotes, @InternalNotes, @NettPriceTotal, @GSTTotal)",
    ...
    public async Task UpdateInvoiceAsync(Invoice inv, List<InvoiceLineItem> lines, string userCode)
    {
        ...
        await _db.ExecuteAsync(@"
            UPDATE Invoices SET
                InvoiceNum = @InvoiceNum, Code = @Code, DivisionId = @DivisionId,
                Qid = @Qid, CompanyId = @CompanyId, ContactId = @ContactId,
                CCompany = @CCompany, InvCompany = @InvCompany, DelCompany = @DelCompany,
    ...
    {
        var dt = await _db.QueryAsync(@"
            SELECT i.InvoiceId, ISNULL(i.InvoiceNum,'') AS InvoiceNum,
                   i.InvoiceDate,
                   ISNULL(co.Company, ISNULL(i.CCompany,'')) AS Company,
                   ISNULL(cn.FirstName,'') AS FirstName,
                   ISNULL(cn.Surname,'')   AS LastName,
                   ISNULL(i.NettPriceTotal,0) AS PriceExGST
            FROM Invoices i
            LEFT JOIN Companies co ON i.CompanyId = co.CompanyId
            LEFT JOIN Contacts  cn ON i.ContactId = cn.ContactId
    ...
    private static Invoice MapInvoice(DataRow r) => new()
    {
        ...
        InvCompany     = r["InvCompany"]?.ToString() ?? "",
