...
<head>
	...
	<style>
		...
		.active-column-4 {width: 250px; text-align: left;}
		...
		.active-column-8 {width: 0px; text-align: left;}
...
function activewidgets_grid(name, oRecordset)
	...
	Do while (Not oRecordset.eof)
		...
		For i=0 to (column_count-1)
			If i = (column_count-1) Then
				rows = rows & """" & activewidgets_html(oRecordset("Comment #"), oRecordset(i), oRecordset("TableId"), oRecordset("ItemId"), oRecordset(i).name) & """ "
			Else
				rows = rows & """" & activewidgets_html(oRecordset("Comment #"), oRecordset(i), oRecordset("TableId"), oRecordset("ItemId"), oRecordset(i).name) & """, "
			End If
		Next
...
function activewidgets_html(Id, s, TableId, ItemId, FieldName)
	...
	If Not IsNull(s) Then
		...
		ElseIf s = "ACTION" Then
			s = "<a href='#' onclick='ViewCommentRecord(""" & Request.Cookies("ClientSettings")("WorkingDir") & """, " & TableId & ", " & ItemId & ");'>View Record</a> | <a href='#' onclick='ReplyToTableComment(""" & Request.Cookies("ClientSettings")("WorkingDir") & """, " & Id & ", " & TableId & ", " & ItemId & ");'>Reply</a> | <a href='" & Request.Cookies("ClientSettings")("WorkingDir") & "/Comments/Del_Proc.asp?CommentId=" & Id & "' target='_parent'>Delete</a>"
		End If
		s = Replace(s, "\", "\\")
		s = Replace(s, """", "\""")
		s = Replace(s, vbCr, "\r")
...
' Execute a SQL query
sql = "SELECT Comments.CommentId As [Comment #], Comments.Date, Users.Name, Tables.Table As [Section], Comments.Comment, Comments.FollowUpDate As [Follow Up Date], Comments.FollowUpComplete As [Follow Up Complete], 'ACTION' As [Action], Tables.PathToView, Tables.TableId, Comments.ItemId FROM Users INNER JOIN (Tables INNER JOIN Comments ON Tables.TableId = Comments.TableId) ON Users.Code = Comments.FromCode WHERE Comments.DateEntered >= #" & dteDateFrom & "# AND Comments.DateEntered <= #" & dteDateTo & "#"
If strCode <> "All" Then
	sql = sql & " AND Comments.FromCode = '" & strCode & "'"
End If
sql = sql & " ORDER BY Comments.[Date] DESC"
