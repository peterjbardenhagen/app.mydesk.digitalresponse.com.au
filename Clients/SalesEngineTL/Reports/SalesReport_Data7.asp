...
%><graph caption='Expenses Budget YTD' showValues='0'>
	<categories>
		<category Name='Jul' />
		<category Name='Aug' />
		<category Name='Sep' />
...
Do
	If Not(arrValues(i) = "") Or lngRunningTotal <> 0 Then
	    lngRunningTotal = lngRunningTotal + CLng(arrValues(i))
		Response.Write "		<set value='" & lngRunningTotal & "' />" & vbCrlf
	Else
...
Do Until i = 12
	If Not(IsNumeric(lngExpensesBudget)) Then
		Response.Write "		<set value='0' />" & vbCrlf
	Else
	    lngRunningExpensesPerMonth = lngRunningExpensesPerMonth + lngExpensesPerMonth
		Response.Write "		<set value='" & lngRunningExpensesPerMonth & "' />" & vbCrlf
	End If
	i = i + 1
Loop
