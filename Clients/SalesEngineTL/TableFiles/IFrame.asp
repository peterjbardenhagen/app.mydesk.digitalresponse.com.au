...
<head>
	...
	<style>
		...
		.active-column-13 {width: 200px; text-align: right;}
		.active-column-14 {width: 120px; text-align: right;}
		.active-column-15 {width: 120px; text-align: left;}
...
function activewidgets_grid(name, oRecordset)
	...
	Do while (Not oRecordset.eof)
		...
		For i=0 to (column_count-1)
			If i = (column_count-1) Then
				rows = rows & """" & activewidgets_html(oRecordset("FileId"), oRecordset("Filename"), oRecordset(i), oRecordset(i).name) & """ "
			Else
				rows = rows & """" & activewidgets_html(oRecordset("FileId"), oRecordset("Filename"), oRecordset(i), oRecordset(i).name) & """, "
			End If
		Next
...
function activewidgets_html(Id, sFilename, s, FieldName)

	If Not IsNull(s) Then
		If IsNumeric(s) And (FieldName = "Margin") Then
			s = FormatNumber(s, 2) & "%"
		ElseIf IsDate(s) Then
			s = FormatDateU2(s, False)
		ElseIf s = "ACTION" Then
			s = "<a href='Files/" & sFilename & "' target='_blank'>Download File</a> | <a href='#' onclick='deleteRecord(" & Id & ");'>Delete</a>"
		End If
		's = Replace(s, "'", "`")
		s = Replace(s, "\", "\\")
		s = Replace(s, """", "\""")
...
' Execute a SQL query
strSql = "SELECT DISTINCT FileId, Description, Filename, Filesize, 'ACTION' As Action FROM TableFiles"
strSql = strSql & " WHERE (TableFiles.DateEntered >= #" & DBDate(dteDateFrom) & "# AND TableFiles.DateEntered < #" & DBDate(dteDateTo) & "#)"
If strKeyword <> "" Then
	strSql = strSql & " AND TableFiles.Description LIKE '%" & strKeyword & "%'"
End If
If strCode <> "All" Then
	strSql = strSql & " AND TableFiles.Code = '" & strCode & "'"
End If
