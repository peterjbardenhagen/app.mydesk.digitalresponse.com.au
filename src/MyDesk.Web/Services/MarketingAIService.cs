...
- Output structured markdown (bullets, headings, tables) when helpful.");
    ...
    /// <summary>
    /// Get marketing insights based on current data
    /// </summary>
    public async Task<MarketingInsights> GetInsightsAsync()
    {
        try
        {
            var cdp = await _data.GetCustomerDataAsync();
            ...
            return new MarketingInsights
            {
                TopRecommendation = $"Focus on top {Math.Min(10, cdp.All.Count)} customers by score for upselling opportunities",
                Opportunities = new List<string>
                {
                    "Expand into regions with highest customer concentration",
                    "Target suppliers with high reliability scores",
                    "Develop campaigns for at-risk customers"
                },
                RecommendedSegments = cdp.SegmentCounts.Keys.ToList()
            };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting marketing insights");
            return new MarketingInsights();
        }
    }

    /// <summary>
    /// Get AI-generated content for marketing materials
    /// </summary>
    public async Task<List<GeneratedContent>> GetGeneratedContentAsync()
    {
        try
        {
            return new List<GeneratedContent>
            {
                new GeneratedContent
                {
                    ...
                    Type = "Email",
                    Content = "Draft email content for Q1 campaign...",
        ...
        {
            _logger.LogError(ex, "Error getting generated content");
            return new List<GeneratedContent>();
        }
    }

    /// <summary>
    /// Process a natural language query about marketing data
    /// </summary>
    public async Task<string> ProcessQueryAsync(string query)
    {
        return await AskMarketingQuestionAsync(query);
    }

    public async Task<MarketingStrategy?> GenerateStrategyAsync(string prompt)
    {
        await Task.Delay(1000);
        return new MarketingStrategy
        {
            Name = "AI Generated Strategy",
            Description = "Strategy generated by AI based on your prompt",
            Timeframe = "12 months",
            Status = "Draft",
            ProgressPercent = 0,
            TotalObjectives = 0,
