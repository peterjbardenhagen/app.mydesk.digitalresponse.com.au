...
%>
		<table width=1000 cellpadding=3 cellspacing=0 border=0 ID="Table1">
			<tr>
				<td valign="top"><span class="TimesHeader">MyExpenses : </span><span style="font-size:18px;"><%= strName %></span><br><br>
				For <% If intCompanyId = 0 Then Response.Write("All companies") Else Response.Write(strCompany) %><br>
...
sql = "SELECT Expenses.*, ExpenseTypes.ExpenseType, Contacts.Company + '<br>' + Contacts.Surname + ', ' + Contacts.FirstName As [Customer] FROM ExpenseTypes INNER JOIN (Users INNER JOIN (Contacts INNER JOIN Expenses ON Contacts.ContactId = Expenses.ContactId) ON Users.Code = Expenses.Code) ON ExpenseTypes.ExpenseTypeId = Expenses.ExpenseTypeId WHERE "
If strCode <> "All" Then
	sql = sql & "Expenses.Code = '" & strCode & "' AND "
End If
If intCompanyId <> 0 Then
	sql = sql & "Contacts.CompanyId = " & intCompanyId & " AND "
End If
...
%>
		...
		<table width="1000" cellpadding=3 cellspacing=0 border=0>
			<tr>
				...
				<th style="font-weight:bold;background-color:#ebeadb;color:black;text-align:left;vertical-align:top;" nowrap>Expense Type</td>
<%
	If strCode = "All" Then
%>
...
%>
				...
				<th style="font-weight:bold;background-color:#ebeadb;color:black;text-align:right;vertical-align:top;text-align:right;">Company Staff</td>
				<th style="font-weight:bold;background-color:#ebeadb;color:black;text-align:right;vertical-align:top;text-align:right;">Non Company Staff</td>
				<th style="font-weight:bold;background-color:#ebeadb;color:black;text-align:left;vertical-align:top;">Corporate Card</td>
...
%>
			...
			<tr>
				...
				<td style="vertical-align:top;" nowrap><%= rs("ExpenseType") %></td>
<%
	If strCode = "All" Then
%>
				<td style="vertical-align:top;" nowrap><%= strName %></td>
<%
	End If
	If intCompanyId = 0 Then
%>
...
%>
				...
				<td style="vertical-align:top;"><%= Replace(Replace(rs("TTLCorporateCard"), True, "Yes"), False, "No") %></td>
