...
function Items_InsertLine(bDivisionManager)
{
	...
	newitem+="				<tr><td style='font-weight:bold;font-size:10px;'>Unit Cost</td><td><input type='text' name='UnitCost" + itemLines + "' id='UnitCost" + itemLines + "' size=1 border=1 style='width:110px;border:1px solid black !important;text-align:right;' value='0.00' onchange='Items_UnitCostChange("+itemLines+")'></td></tr>";
...
function Items_InsertLine_WithData(itemLine, itemQuantity, itemDays, itemUnits, itemType, itemProductCode, itemDescription, itemProductId, itemUnitCost, itemMinNettPrice, itemNettPrice, itemUnitCostSubTotal, itemMinExtNettPrice, itemExtNettPrice, bDivisionManager, bPerUnitPerDay)
{
	...
	newitem+="		<td width=220 class='Quote_Item_TD' align='right'><img src='/Images/Spacer.gif' width=220 height=1 border=0 alt=''><br>";
	newitem+="			<table cellpadding=1 cellspacing=0 border=0>";
	newitem+="				<tr><td style='font-weight:bold;font-size:10px;'>Unit Cost</td><td><input type='text' name='UnitCost" + itemLines + "' id='UnitCost" + itemLines + "' size=1 border=1 style='width:110px;border:1px solid black !important;text-align:right;' value='"+formatDecimal(itemUnitCost)+"' onchange='Items_UnitCostChange("+itemLines+")'></td></tr>";
...
function Items_FormatLine(itemLine) {
	...
	*/
	...
	var unitCost = document.getElementById("UnitCost"+itemLine).value;
	var nettPrice = document.getElementById("NettPrice"+itemLine).value;
	if(unitCost > 0 && nettPrice > 0) {
		document.getElementById("ExtNettPrice"+itemLine).value = formatDecimal(unitCost * nettPrice);
	}
	//Quotes_CalcAll();
...
function Items_MarginChange(itemLine) {
	...
	// Fix bug when changing margin Ext Nett Price doesn't add up
...
function PostRender() {
	var i = 2;
	while (i < itemLines+1) {
		document.getElementById("UnitCost" + i).style.border = "1px solid black";
		console.log('Set border = 1 for item line = ' + i);
		i++;
	}
}