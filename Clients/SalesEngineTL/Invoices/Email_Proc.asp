...
Sub SendMail(strFromEmail, strToEmail, strSubject, strBody)
	...
	With objCDO
		...
		.BCC = "admin@techlight.com.au"
		.Subject = strSubject
		.HtmlBody = strBody
		.AddAttachment(Server.MapPath(Request.Cookies("ClientSettings")("WorkingDir") & "/Invoices/Files") & "\Invoice.pdf")
		.Send
	End With

	' Audit trail
	sql = "Insert Into InvoiceAudit (InvoiceId, Code, Action, DateEntered) Values (" & lngInvoiceId & ", '" & Request.Cookies("UserSettings")("Code") & "', 'Invoice issued by email to " & strToEmail & "', '" & ServerToEST(Now()) & "')"
	dbConn.Execute(sql)
End Sub
...
<html>
	...
	<body Marginheight=0 Marginwidth=2 topMargin=0 leftMargin=2>
		...
		<table class="NoPrint">
			<tr>
				<td class="Header4">Email Invoice / Result</td>
			</tr>
		</table>
		<br class="NoPrint">
...
%>
				<td style="color:red;">Invoice email not sent successfully. Please try again later. Error : <%= err.Description %></td>
<%
Else
%>
				<td>Invoice email sent successfully</td>
<%
End If
%>
			</tr>
