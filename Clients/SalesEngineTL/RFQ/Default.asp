		...
		<tr>
			<td>
				...
				<table width="100%" cellpadding=0 cellspacing=0 border=0 ID="Table4">
					<tr>
						<td><span class="Header2"><a href="/Portal.asp" class="Header2">Home</a> / Request For Quote /></span></td>
						<td align="right"><a href="<%= Request.Cookies("ClientSettings")("WorkingDir") %>/RFQ/Add.asp" class="Header2">Add Request For Quote</a></td>
					</tr>
				</table>
				<table width=760>
					<tr>
...
sql = "Select DistinctRow Companies.CompanyId, Companies.Company, Divisions.RFQ As HasRFQ FROM Divisions INNER JOIN (Companies INNER JOIN Contacts ON Companies.CompanyId = Contacts.CompanyId) ON Divisions.DivisionId = Companies.DivisionId Where Companies.CompanyId <> 142 And SupplierCode <> '' And Companies.DivisionId In (" & Request.Cookies("DivisionIdsAccess")("RFQ") & ") And Contacts.Code In (" & GetAccessCodesList(Request.Cookies("UserSettings")("Code"), Request.Cookies("UserSettings")("UserTypeID")) & ") And Divisions.RFQ = 1 Order By Companies.Company"
...
sql = "SELECT * FROM Divisions WHERE RFQ = True AND DivisionId In (" & Request.Cookies("DivisionIdsAccess")("RFQ") & ") ORDER BY Division"
...
%>
				...
				<table width=100% cellpadding=0 cellspacing=0 border=0 ID="Table2">
					<tr>
						<td>
						<iframe width=100% height=250 name="MyIFrame" src="IFrame.asp?Cache=<%= rnd() %>&Sort=<%= strSort %>&CurPage=<%= CurPage %>&Code=<%= strFilter_Code %>&Company=All&DateFrom=<%= dteDateFrom %>&DateTo=<%= dteDateTo %>&DivisionId=<%= intSelDivisionId %>"></iframe>
						</td>
					</tr>
				</table>
			</td>
