...
sql = "Select @@IDENTITY As NewContactId"
Set rsNew = dbConn.Execute(sql)
intNewContactId = rsNew("NewContactId")
...
<html>
	<head>
		...
		<script language="javascript">
			try {
				CreateNewContact_UpdateSelect('<%= intNewContactId %>', 'New Contact : <%= strFirstName & " " & strSurname %>', '<%= strField %>');
				alert('Contact added, and selected.');
				window.close();
			} catch(e) {
				alert('Contact added');
				window.close();
