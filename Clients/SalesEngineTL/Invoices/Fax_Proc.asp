...
strSubject = rsDi("Division") & " (Invoice # " & lngInvoiceId & ")"

strBody = "Dear " & strAttention & ",<br><br>"
strBody = strBody & "Please find attached Invoice (Invoice # " & lngInvoiceId & ") "
strBody = strBody & ".<br><br>"
If Trim(strNotes) <> "" Then strBody = strBody & "<b>Notes:</b> " & Replace(strNotes, vbcrlf, "<br>") & "<br><br>"
strBody = strBody & GetUserContactDetailsFax(rsInv("QDivisionId"), rsInv("Code"))
...
Sub SendMail(strFromFax, strToFax, strSubject, strBody)
	...
	With objCDO
		...
		.Subject = strSubject
		.HtmlBody = strBody
		.AddAttachment(Server.MapPath(Request.Cookies("ClientSettings")("WorkingDir") & "/Invoices/Files") & "\" & lngInvoiceId & "\Invoice.html")
		.Send
	End With

	' Audit trail
	sql = "Insert Into InvoiceAudit (InvoiceId, Code, Action, DateEntered) Values (" & lngInvoiceId & ", '" & Request.Cookies("UserSettings")("Code") & "', 'Invoice issued by fax to " & strToFax & "', '" & ServerToEST(Now()) & "')"
	dbConn.Execute(sql)
End Sub
...
<html>
	...
	<body Marginheight=0 Marginwidth=2 topMargin=0 leftMargin=2>
		...
		<table class="NoPrint">
			<tr>
				<td class="Header4">Fax Invoice / Result</td>
			</tr>
		</table>
		<br class="NoPrint">
...
%>
				<td style="color:red;">Invoice fax not sent successfully. Please try again later.</td>
<%
Else
%>
				<td>Invoice fax sent successfully</td>
<%
End If
%>
			</tr>
