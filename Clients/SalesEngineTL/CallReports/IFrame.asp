...
%>
		...
		.active-column-4 {width: 100px; text-align: left;}
		...
		.active-column-10 {width: 60px; text-align: left;}
		.active-column-11 {width: 60px; text-align: left;}
		.active-column-12 {width: 60px; text-align: left;}
...
function activewidgets_grid(name, oRecordset)
	...
	Do while (Not oRecordset.eof)
		...
		For i=0 to (column_count-1)
			If i = (column_count-1) Then
				rows = rows & """" & activewidgets_html(oRecordset("Call Report #"), oRecordset(i), oRecordset(i).name) & """ "
			Else
				rows = rows & """" & activewidgets_html(oRecordset("Call Report #"), oRecordset(i), oRecordset(i).name) & """, "
			End If
		Next
...
function activewidgets_html(Id, s, FieldName)
	...
	If Not IsNull(s) Then
		...
		ElseIf s = "ACTION" Then
			s = "<a href='#' onclick='ViewCallReport(""" & Request.Cookies("ClientSettings")("WorkingDir") & """, " & Id & ");'>View</a> | <a href='" & Request.Cookies("ClientSettings")("WorkingDir") & "/CallReports/Edit.asp?CallReportId=" & Id & "' target='_parent'>Edit</a> | <a href='#' onclick='deleteRecord(" & Id & ");'>Delete</a>"
...
' Execute a SQL query
sql = "SELECT CallReports.CallReportId AS [Call Report #], Users.Name AS [Originator], Contacts_WithCustomersAndSuppliers_V2.CompanyName + ' - ' + Contacts_WithCustomersAndSuppliers_V2.Surname + ', ' + Contacts_WithCustomersAndSuppliers_V2.FirstName AS [Contact], 'ACTION' AS [Action], CallReports.CallPurpose AS [Call Purpose], CallReports.Comment, CallReportTypes.CallReportType As [Call Report Type], CallReports.DateEntered AS [Date of Call], CallReports.RealDateEntered AS [Real Date Entered], 'HISTORY' As [History] FROM (Users INNER JOIN (Contacts_WithCustomersAndSuppliers_V2 INNER JOIN CallReports ON Contacts_WithCustomersAndSuppliers_V2.ContactId = CallReports.ContactId) ON Users.Code = CallReports.Code) INNER JOIN CallReportTypes ON CallReports.CallReportTypeId = CallReportTypes.CallReportTypeId"
...
sql = sql & " DateEntered >= #" & dteDateFrom & "# And DateEntered < #" & dteDateTo & "#"
sql = sql & " ORDER BY CallReports.[DateEntered] DESC"
