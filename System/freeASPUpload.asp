...
Class FreeASPUpload
    ...
    Public Function ChangeName(filename)
        Dim suffix, newfilename
        dim i, exists
        suffix = Right(filename, 4)
        suffix = Replace(suffix, ".", "")
        i = 1
        exists = true
        do until (exists = false)
            newfilename = Left(filename, Len(filename)-4) & "_" & i & "_." & suffix
            Dim fso
            Set fso = CreateObject("Scripting.FileSystemObject")
            If (Not fso.FileExists(path & newfilename)) Then
                exists = false
            End If
            Set fso = nothing
            i = i + 1
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
            ...
            ' First make sure there are no files in the library of the same filename
            Set rs = Server.CreateObject("ADODB.RecordSet")
     		sql = "Select Top 1 * From Files Where Filename = '" & fileItem.FileName & "'"
            Set rs = dbConn.Execute(sql)

            if(Not (rs.BOF and rs.EOF)) Then
                Dim fso
                Set fso = CreateObject("Scripting.FileSystemObject")
                If (fso.FileExists(path & fileItem.FileName)) Then
                    Dim exists, newfilename, i, suffix
                    exists = true
                    newfilename = ""
                    i = 0
                    suffix = Right(fileItem.FileName, 4)
                    suffix = Replace(suffix, ".", "")
                    Do Until (exists = false or i = 10)
                        i = i + 1
                        newfilename = Left(fileItem.FileName, Len(fileItem.FileName)-4) & "_" & i & "_." & suffix
                        exists = fso.FileExists(path & newfilename)
                    Loop
                    fso.MoveFile path & fileItem.FileName, path & newfilename
                    sql = "Update Files Set Filename = '" & newfilename & "' Where Filename = '" & fileItem.FileName & "'"
                    dbConn.Execute(sql)
                End If
                Set fso = Nothing
            End If
