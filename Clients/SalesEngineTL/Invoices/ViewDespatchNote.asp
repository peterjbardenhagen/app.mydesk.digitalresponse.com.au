...
If boolPrint Then
	' Audit trail
	sql = "Insert Into InvoiceAudit (InvoiceId, Code, Action, DateEntered) Values (" & lngInvoiceId & ", '" & Request.Cookies("UserSettings")("Code") & "', 'Despatch Note Printed', '" & ServerToEST(Now()) & "')"
	dbConn.Execute(sql)
End If
...
%>
		<script>
			function invoiceAddress() {
				var s;
				var sState;
				s = '<%= Replace(rsInv("InvCompany"),"'","''") %>' + '<br>';
				sState = '<%= Replace(rsInv("InvState"),"'","''") %>';
				s += '<%= Replace(rsInv("InvAddress1"),"'","''") %>' + '<br>';
				if(<%= Len(Replace(rsInv("InvAddress2"),"'","''")) %>>0){
					s += '<%= Replace(rsInv("InvAddress2"),"'","''") %>' + '<br>';
				}
				s += '<%= Replace(rsInv("InvSuburb"),"'","''") %>' + ' ' + sState + ' ' + '<br>' + '<%= Replace(rsInv("InvCountry"),"'","''") %>' + ' ' + '<%= rsInv("InvPostCode") %>' + '\n';
				//InvoiceAddress_Select_InJobOrder(s, '<%= Replace(rsInv("InvCompany"),"'","''") %>', '<%= Replace(rsInv("InvAddress1"),"'","''") %>', '<%= Replace(rsInv("InvAddress2"),"'","''") %>', '<%= Replace(rsInv("InvSuburb"),"'","''") %>', '<%= rsInv("InvStateId") %>', '<%= Replace(rsInv("InvState"),"'","''") %>', '<%= rsInv("InvPostCode") %>', '<%= Replace(rsInv("InvCountry"),"'","''") %>');
				//window.close();
				return s;
			}
			invoiceAddress();
			...
			function deliveryAddress() {
				var s;
				var sState;
				s = '<%= Replace(rsInv("DelCompany"),"'","''") %>' + '<br>';
				sState = '<%= Replace(rsInv("DelState"),"'","''") %>';
				s += '<%= Replace(rsInv("DelAddress1"),"'","''") %>' + '<br>';
				if(<%= Len(Replace(rsInv("DelAddress2"),"'","''")) %>>0){
					s += '<%= Replace(rsInv("DelAddress2"),"'","''") %>' + '<br>';
				}
				s += '<%= Replace(rsInv("DelSuburb"),"'","''") %>' + ' ' + sState + ' ' + '<br>' + '<%= Replace(rsInv("DelCountry"),"'","''") %>' + ' ' + '<%= rsInv("DelPostCode") %>' + '\n';
				//deliveryAddress_Select_InJobOrder(s, '<%= Replace(rsInv("DelCompany"),"'","''") %>', '<%= Replace(rsInv("DelAddress1"),"'","''") %>', '<%= Replace(rsInv("DelAddress2"),"'","''") %>', '<%= Replace(rsInv("DelSuburb"),"'","''") %>', '<%= rsInv("DelStateId") %>', '<%= Replace(rsInv("DelState"),"'","''") %>', '<%= rsInv("DelPostCode") %>', '<%= Replace(rsInv("DelCountry"),"'","''") %>');
				//window.close();
				return s;
			}
			deliveryAddress();
...
%>
		<table width="100%" bgcolor="#DDDDDD" class="NoPrint" width="100%" cellpadding=5 cellspacing=0 border=0 ID="Table3">
			<tr>
				<td>
				<!--#include file="NavBarDespatchNote.asp"-->
				</td>
			</tr>
		</table>
		<br class="NoPrint">
		<table class="NoPrint" ID="Table13">
			<tr>
				<td class="Header4">View Despatch Note</td>
			</tr>
		</table>
		<br class="NoPrint">
<%
...
%>
			...
			<tr>
				<td colspan=2>
				<span style="font-size:24px;">DESPATCH NOTE</span>
				</td>
			</tr>
			<tr>
				<td colspan=2><br></td>
			...
			<tr>
				<td class="Times2Bold" valign="top" width="50%">
					<table align="center" width="100%" border="0" bordercolor="#00000" cellpadding="3" cellspacing="0" ID="Table11">
						...
						<tr>
							<td style="font-weight:bold;vertical-align:top;width:60px;">Job #:</td>
							<td><%= rsInv("JobOrderId") %></td>
						</tr>
					</table>
				</td>
				<td class="Times2Bold" valign="top" width="50%">
					<table align="center" width="100%" border="0" bordercolor="#00000" cellpadding="3" cellspacing="0" ID="Table8">
						...
						<tr>
							<td style="font-weight:bold;vertical-align:top;width:105px;">Customer Order #:</td>
				...
				<td valign="top" width="50%">
					<table align="center" width="100%" border="0" bordercolor="#00000" cellpadding="3" cellspacing="0" ID="Table4">
						<tr>
							...
							<script language="javascript">document.write(invoiceAddress());</script>
							</td>
						</tr>
					</table>
				</td>
				<td valign="top" width="50%">
					<table align="center" width="100%" border="0" bordercolor="#00000" cellpadding="3" cellspacing="0" ID="Table5">
						<tr>
							...
							<script language="javascript">document.write(deliveryAddress());</script>
							</td>
						</tr>
<%
